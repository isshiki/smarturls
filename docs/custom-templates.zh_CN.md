...
> ⚠️ **关于 `$nl` 的注意事项**：`$nl` 标记可用于**复制**自定义模板中，以在生成的文本中插入换行符。但是，它在**从文本打开**侧的自定义模板中**不受支持**，因为该功能逐行处理输入。因此，如果您将在复制侧使用 `$nl` 的模板重新用作打开自定义模板，它将不会以相同的方式运行。如果您希望复制和打开共享同一模板，请在打开模板中避免使用 `$nl`，或使用**智能（自动检测）**模式。
