...
> ⚠️ **關於 `$title(html)` 的注意事項**：`$title(html)` 標記僅在**複製**自訂範本中受支援。它用於在產生的文字中插入 HTML 轉義的頁面標題。它在**從文字開啟**側的自訂範本中**不受支援**。如果您將包含 `$title(html)` 的範本重新用作開啟自訂範本，此標記將不會被處理。對於開啟範本，請改用 `$title`。
...
> ⚠️ **關於查詢參數標記的注意事項**：查詢參數標記（如 `$v`、`$id`、`$tag` 等）僅在**複製**自訂範本中進行評估。它們允許您在複製輸出中插入或格式化從 URL 查詢字串獲取的值。這些標記在**從文字開啟**側的自訂範本中**不會被評估**。從文字開啟自訂範本不會讀取或過濾查詢參數；它們僅使用模式在貼上的文字中定位 `$url`。
...
> ⚠️ **關於條件區塊的注意事項**：條件區塊（如 `{{q=v: ...}}` 或 `{{q=v,t: ...}}`）僅在**複製**自訂範本中受支援。它們允許您根據 URL 查詢參數包含或排除部分輸出。條件區塊在**從文字開啟**側的自訂範本中**不受支援**。從文字開啟範本不會評估這些條件，也無法根據它們過濾要開啟的 URL。如果您需要控制要開啟哪些 URL，請在來源文字中使用過濾，或改用**智慧（自動偵測）**模式。
