...
> ⚠️ **`$title(html)`에 대한 참고 사항**: `$title(html)` 토큰은 **복사** 커스텀 템플릿에서만 지원됩니다. 생성된 텍스트에 HTML 이스케이프된 페이지 제목을 삽입하는 데 사용됩니다. **텍스트에서 열기** 측의 커스텀 템플릿에서는 **지원되지 않습니다**. `$title(html)`을 포함하는 템플릿을 열기 커스텀 템플릿으로 재사용하면 이 토큰은 처리되지 않습니다. 열기 템플릿의 경우 대신 `$title`을 사용하세요.
...
> ⚠️ **쿼리 매개변수 토큰에 대한 참고 사항**: `$v`, `$id`, `$tag` 등의 쿼리 매개변수 토큰은 **복사** 커스텀 템플릿에서만 평가됩니다. 이를 사용하면 URL의 쿼리 문자열에서 가져온 값을 복사 출력에 삽입하거나 형식을 지정할 수 있습니다. 이러한 토큰은 **텍스트에서 열기** 측의 커스텀 템플릿에서는 **평가되지 않습니다**. 텍스트에서 열기 커스텀 템플릿은 쿼리 매개변수를 읽거나 필터링하지 않으며, 붙여넣은 텍스트에서 `$url`을 찾기 위해 패턴만 사용합니다.
...
> ⚠️ **조건부 블록에 대한 참고 사항**: `{{q=v: ...}}` 또는 `{{q=v,t: ...}}`와 같은 조건부 블록은 **복사** 커스텀 템플릿에서만 지원됩니다. 이를 사용하면 URL 쿼리 매개변수에 따라 출력의 일부를 포함하거나 제외할 수 있습니다. 조건부 블록은 **텍스트에서 열기** 측의 커스텀 템플릿에서는 **지원되지 않습니다**. 텍스트에서 열기 템플릿은 이러한 조건을 평가하지 않으며 이를 기반으로 열 URL을 필터링할 수 없습니다. 열 URL을 제어해야 하는 경우 소스 텍스트에서 필터링을 사용하거나 **스마트(자동 감지)** 모드를 사용하세요.
